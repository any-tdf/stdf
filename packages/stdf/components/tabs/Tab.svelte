...
{#if overflow && layout === 'h'}
	...
	>
		{#if lineType}
			<div class="absolute h-0.5 bg-black/5 dark:bg-white/5 w-full bottom-0" style="width: {itemW * labels.length}px"></div>
		{/if}
		<div
...
{:else}
	...
	>
		{#if lineType && layout !== 'v'}
			<div class="absolute h-0.5 bg-black/5 dark:bg-white/5 w-full bottom-0" style="width: {tabW - 4}px"></div>
		{/if}
		<div
