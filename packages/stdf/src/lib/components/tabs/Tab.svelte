<script lang="ts">
	...
	let showIndexs = $derived(new Array(showNum + 1).fill(0).map((_, index) => index + showIndexsOffset));
...
{#if overflow && layout === 'h'}
	<div
		...
		bind:this={ofDom}
		class="{lineType ? '' : 'bg-black/5 dark:bg-white/10'} no-scrollbar relative overflow-auto scroll-smooth p-0.5 {radius ? radiusObj[radius] : 'rounded-(--radius-form)'} {mxClass[mx] || mxClass['2']} {injClass}"
	>
		{#if lineType}
		...
		<div
			class="{lineType ? '' : 'dark:shadow-xs shadow-sm dark:shadow-white/10'} absolute transition-all {durationObj[duration] ||
				durationObj.base} {radius ? radiusObj[radius] : 'rounded-(--radius-form)'} {lineType
				? 'bg-primary dark:bg-dark bottom-0'
				: 'bg-bg-highlight dark:bg-bg-highlight-dark'} {activeInjClass}"
		...
		<div class="relative flex whitespace-nowrap" style="width: {itemW * labels.length + 2}px;">
			{#each labels as label, i (i)}
				<button
					class="flex shrink-0 justify-center overflow-hidden py-1 font-medium {love ? 'text-lg' : 'text-sm'} leading-6 {radius ? radiusObj[radius] : 'rounded-(--radius-form)'} {tabInjClass} {i === active ? activeTabInjClass : ''}"
...
{:else}
	<div
		...
		class="{lineType && layout !== 'v' ? '' : 'bg-black/5 dark:bg-white/10'} relative p-0.5 {radius ? radiusObj[radius] : 'rounded-(--radius-form)'} {mxClass[
			mx
		] || mxClass['2']} {injClass}"
	>
		...
		<div
			class="{lineType && layout !== 'v' ? '' : 'dark:shadow-xs shadow-sm dark:shadow-white/10'} absolute transition-all {durationObj[
				duration
			] || durationObj.base} {radius ? radiusObj[radius] : 'rounded-(--radius-form)'} {lineType && layout !== 'v'
				? 'bg-primary dark:bg-dark bottom-0'
				: 'bg-bg-highlight dark:bg-bg-highlight-dark'} {activeInjClass}"
			...
			{#each labels as label, i (i)}
				<button
					class="flex flex-1 justify-center {layout === 'h' ? 'py-1' : 'py-2'} overflow-hidden font-medium {love
						? 'text-lg'
						: 'text-sm'} leading-6 {radius ? radiusObj[radius] : 'rounded-(--radius-form)'} {tabInjClass} {i === active ? activeTabInjClass : ''}"
					onclick={() => clickTabFun(i)}
				>
					{#if label.icon}
						<div class="mr-0.5 {durationObj[duration] || durationObj.base}">
