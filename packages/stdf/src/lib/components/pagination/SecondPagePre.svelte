<script lang="ts">
	...
	let { pageCol = 3, Pages = [], maxShowPage = 7, radius = 'md', type = 'bold', bg = 'white', onclickItem }: Props = $props();
...
{#if Pages.length > 0}
	<div
		...
		style="width: calc({(pageCol / (maxShowPage + 2)) * 100}% + 8px);left:{(2.5 / (maxShowPage + 2)) * 100}%;"
	>
		<div
