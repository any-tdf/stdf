<script>
  ...
  const pushFun = (child) => {
    ...
    localStorage.setItem("lastView", child.nav);
    push(`/${child.nav}`);
  };
  ...
  onMount(() => {
    ...
    const dom = document.getElementById(localStorage.getItem("lastView"));
    dom.style.color = "red";
  });
</script>
...
<div class="h-14 mt-8 mb-2 flex flex-col justify-center items-center">
  <svg viewBox="0 0 90 80">
    ...
    />
    <path
      class="fill-dark dark:fill-primary"
...
<div class="flex flex-col py-4">
  <div>
    {#each menuList as menu}
      <div class="text-sm px-4 mt-8 mb-2 text-gray6">
  ...
  <div class="p-4">
    {isZh
      ? "当前组件总数："
      : "Current number of components: "}{menuListArr.length}
  </div>
  <div
    class="flex text-xs justify-around p-4 text-primary dark:text-dark underline"
  >
